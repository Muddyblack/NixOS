...
| `kernel.unprivileged_bpf_disabled = 1` | Monitoring tools need sudo | Tools like `bpftrace`, some `btop` features, `rustnet`/`netscanner` require elevated privileges. Acceptable tradeoff but warn the user. |
